...
//		int index{0};
//		for(int i=0;index<mWave.data.size;i+=2)
//			samples[index++] = static_cast<short int>(static_cast<unsigned char>(mWave.data.samples[i])|
//								static_cast<unsigned char>(mWave.data.samples[i+1])<<8 );
	}
}
Wave::Wave()
{
